...
const MAKER_ORDER = ["seeed", "rakwireless"] as const;
...
function getMakerOrder(makerSlug: string) {
  const index = MAKER_ORDER.indexOf(makerSlug as (typeof MAKER_ORDER)[number]);

  if (index === -1) {
    return MAKER_ORDER.length;
  }
...
function compareDevices(a: ResolvedDeviceDocument, b: ResolvedDeviceDocument) {
  const makerOrder = getMakerOrder(a.makerSlug) - getMakerOrder(b.makerSlug);
...
export function getLandingDeviceSections(): DeviceShowcaseSectionSpec[] {
  ...
  for (const device of allDevices) {
    ...
    items.push({
      ...
      primaryHref: device.primaryHref,
      aliExpressSearch: device.aliExpressSearch,
      url: device.url,
    });
  }
...
export function aliExpressSearchUrl(query: string) {
